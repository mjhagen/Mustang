    ...
    <cfloop array="#local.activity#" index="local.logEntry">
      ...
      <li class="list-group-item list-group-item-#local.cssClass#">
        ...
        <cfif local.linkToEntity and len( trim( local.loggedEntityID ))>
          <a class="btn btn-xs btn-primary pull-right" style="margin-left:5px;" href="#buildURL( 'admin:' & local.loggedEntityName & '.view?#local.loggedEntityName#id=#local.loggedEntityID#' )#">#i18n.translate( 'view-item' )#</a>
        </cfif>
        <a class="btn btn-xs btn-primary pull-right" style="margin-left:5px;" href="#buildURL( 'admin:logentry.view?logentryID=#local.logEntry.getID()#' )#">#i18n.translate( 'view-logentry' )#</a>
