...
<cfset local.singularColumnName = structKeyExists( local.column, "singularName" ) ? local.column.singularName : local.column.name />
...
<cfoutput>
  ...
  <cfif local.column.fieldtype contains "many">
    <cfif structKeyExists( local.column, "inlineedit" )>
      <cfif local.column.fieldtype contains "to-one">
      ...
      <cfelseif local.column.fieldtype contains "to-many">
        <cfif structKeyExists( local.column, "saved" )>
          ...
          <div class="inlineblock"#arrayLen( local.savedEntities )?'':' style="display:none;"'#>
            <table class="table table-condensed">
              <tbody id="saved-#local.singularColumnName#">
                  ...
                  <tr class="inline-item">
                    <cfloop array="#local.fieldsToDisplay#" index="local.fieldToDisplay">
                      <cfif not isSimpleValue( local.fieldToDisplay )>
                        ...
                        <cfset local.fieldToDisplay = view( 'form/view/field', local.fieldparameters ) />
                      </cfif>
        ...
        <div id="#local.column.name#_inlineedit" class="inlineedit">
          <cfset local.appendFK = structKeyExists( rc, "#rc.entity#id" ) ? "&fk=#rc[ rc.entity & 'id' ]#" : "" />
          <a href="#buildURL( "#local.column.entityName#.new?modal=1#local.appendFK#&source=#local.column.fkColumn#" )#" class="btn btn-sm btn-primary inlineedit-modal-trigger" data-target="##modal-dialog" data-field="#local.singularColumnName#">#i18n.translate( 'add-#local.column.name#' )#</a>
        </div>
      </cfif>
            ...
            <div id="collapse_#local.formElementName#" class="panel-collapse collapse">
              <div class="panel-body">
                <cfloop array="#local.checkboxes#" index="local.checkbox">
                  ...
                  <div class="#structKeyExists( local.column, 'affected' )?'affected #local.checkbox.getName()#':''#">#view( "form/edit/recursive-checkbox", local.viewOptions )#</div>
                </cfloop>
              </div>
            </div>
          </div>
        ...
        <cfloop array="#local.checkboxes#" index="local.option">
          ...
          <div class="form-check">
            <label class="form-check-label">
              <input class="form-check-input" type="checkbox" name="#local.formElementName#" value="#local.option.getID()#"#local.checked##local.required# />
              #structKeyExists( local.column, "translateOptions" )?i18n.translate(local.option.getName()):local.option.getName()#
            </label>
          </div>
        </cfloop>
      </cfif>
      ...
      <cfif structKeyExists( local.column, "hierarchical" )>
        ...
        <cfloop array="#local.selects#" index="local.select">
          ...
          #view( 'form/edit/select', {
            ...
            "contents"  = view( "form/edit/recursive-option", local.viewOptions )
          })#
        </cfloop>
      <cfelse>
  ...
  <cfelseif local.column.ORMType eq "boolean">
    #view( 'form/edit/checkbox', {
      "column" = local.column,
      "formElementName" = local.formElementName,
  ...
  <cfelse>
    ...
    <cfswitch expression="#local.column.formfield#">
      ...
      <cfdefaultcase>
        <cfif structKeyExists( local.column , "editor" )>
          #view( "form/#local.column.editor#", {
            "id" = "#local.idPrepend##local.column.name#",
            "name" = local.formElementName,
        ...
        <cfelseif local.column.formfield eq "datepicker">
          <cfset local.fieldAttributes = ' data-date-format="yyyy-mm-dd HH:ii:00"' />
          <cfif isDate( local.column.saved )>
            <cfset local.column.saved = "#dateFormat( local.column.saved, 'yyyy-mm-dd' )# #timeFormat( local.column.saved, 'HH:mm:00' )#" />
            <cfif year( local.column.saved ) gt 10000>
              <cfset local.column.saved = "infinite" />
            </cfif>
            <cfset local.fieldAttributes &= ' data-date="#local.column.saved#"' />
          </cfif>
          <div class="input-group input-append date datepicker"#local.fieldAttributes#>
            <input class="form-control" name="#local.formElementName#" size="16" type="text" value="#local.column.saved#" readonly>
            <div class="input-group-addon add-on"><i class="fa fa-calendar"></i></div>
          </div>
        <cfelse>
          <cfset local.fieldAttributes &= ' class="form-control #local.column.formfield#" name="#local.formElementName#"' />
